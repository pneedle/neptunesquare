...
echo "<option label=\"-select the orb-\" value=\"\"></option>";
//echo "<option value=\"0.33\" >0 degrees 20 minutes</option>";
echo "<option value=\"1.0\" >1 degree 0 minutes or less</option>";
echo "<option value=\"3.0\" >3 degrees 0 minutes or less</option>";
echo "<option value=\"6.0\" >6 degrees 0 minutes or less</option>";
echo "</select><br />";
...
if(isset($_POST['submit']) && ($_POST['full_name'] != NULL) && ($_POST['orb'] != NULL)) {
	...
	echo "<hr /><p>Click name for natal information: <a href=\"natal/" . $full_name . "\">" . $name_first . " " . $name_last . "<a/><br />";
	echo "Orb: " . $orb . " degrees<br /></p>";
	// echo 'Node ID: ' . $nid . '<hr />';
	...
	for ($i=0; $i<count($a); $i++) {
		for ($j=0; $j<count($b); $j++) {
			$aspect = ab($a[$i], $b[$j]);
			$numeric_angle = round(abs(($a[$i] - $b[$j])),1);
			
			if (($aspect != NULL) && ($nid != $daily_id)) { //} && ($i <= $j)) { // lists transits for the individual (leaving out 0 degrees)
